        ...
        with patch.object(llm_client, 'generate_text') as mock_generate:
            ...
            def generate_text_side_effect(prompt):
                if "简历文本中提取元数据" in prompt:
                    ...
                    return '{"keywords": ["Python"], "required_skills": ["Python"], "min_experience_years": 3, "required_education": null, "required_industries": [], "preferred_industries": [], "salary_range": null, "locations": [], "required_languages": [], "required_certifications": [], "custom_conditions": null, "preferred_skills": []}'
                else:
            ...
            # Mock向量存储管理器的方法
            vector_store_manager.get_collection.return_value = MagicMock()
            vector_store_manager.create_collection.return_value = MagicMock()
            vector_store_manager.add_documents.return_value = None
            vector_store_manager.query_collection.return_value = {
                "ids": [["resume_001"]],
                "documents": [["张三的简历，拥有5年Python开发经验，熟悉Django框架"]],
                "metadatas": [[{
                    "name": "张三",
                    "work_experience": [
                        {
            ...
            # 验证向量存储管理器的方法被调用
            vector_store_manager.get_collection.assert_called()
            vector_store_manager.add_documents.assert_called()
            ...
            assert retrieved_resumes[0]["id"] == "resume_001"
            ...
            # 10. 验证缓存功能
            # 重新提取相同的简历元数据，应该从缓存中获取
            metadata_cached = metadata_extractor.extract_metadata(resume_text)
            assert metadata_cached.name == "张三"
