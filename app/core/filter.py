...
class HardFilter:
    ...
    def _filter_by_skills(self, resumes: List[Dict[str, Any]], required_skills: List[str]) -> List[Dict[str, Any]]:
        ...
        for resume in resumes:
            ...
            # 技能匹配采用阈值机制（匹配 >= 50% 即通过），避免 query_parser 抽取过多
            # 必需技能导致硬性过滤误杀简历。精确的技能评分交由 scorer 层处理。
            fulltext = self._resume_text(resume)
            match_count = sum(1 for skill in required_skills if self._matches(skill, skills, fulltext))
            threshold = max(1, len(required_skills) * 0.5)
            meets_requirement = match_count >= threshold
