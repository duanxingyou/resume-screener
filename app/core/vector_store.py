...
class VectorStoreManager:
    ...
    def __init__(self, persist_directory: Optional[str] = None, embedding_model: Optional[str] = None):
        ...
        embedding_kwargs = {
            ...
            "check_embedding_ctx_length": False,  # 智谱API不支持token ID输入，需传原始文本
        }
    ...
    def add_documents(
        ...
        """
        添加文档到指定集合。如果文档已存在，则更新。分批调用避免智谱 API 超限。

        Args:
            collection_name (str): 集合名称
            documents (List[str]): 文档列表
            metadatas (Optional[List[Dict[str, Any]]], optional): 元数据列表。默认为 None。
            ids (Optional[List[str]], optional): 文档ID列表。默认为 None。
        """
        collection = self.get_collection(collection_name)

        # 序列化元数据：ChromaDB 要求所有元数据值都是基本类型，不能嵌套 list/dict
        # 因此将复杂结构 JSON 序列化后存储
        ...
        try:
            # 手动生成嵌入向量，分批处理避免智谱 API token 超限
            logger.info(f"Generating embeddings for {len(documents)} documents")
            embeddings = []
            batch_size = 1  # 每次处理1篇简历，避免token超限
            for i in range(0, len(documents), batch_size):
                batch = documents[i:i+batch_size]
                # 截断过长的文档，避免智谱 Embedding API 返回 400 错误
                truncated_batch = [doc[:4000] if len(doc) > 4000 else doc for doc in batch]
                logger.debug(f"Processing batch {i//batch_size + 1}/{(len(documents) + batch_size - 1)//batch_size}")
                batch_embeddings = self.embeddings.embed_documents(truncated_batch)
