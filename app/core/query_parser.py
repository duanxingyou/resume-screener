...
class QueryParser:
    ...
    def parse_query(self, query_text: str) -> QueryMetadata:
        ...
        try:
            ...
            # 后处理：将"不限"等语义等同于"无限制"的字段值转为空列表，
            # 避免硬性过滤时误杀简历（如 locations=['不限'] 会匹配不到具体城市）
            _unbounded_values = {"不限", "无", "无限制", "任何", "任意", "所有", "全部"}
            for field in ["locations", "required_languages", "required_certifications",
                          "required_industries", "preferred_industries"]:
                values = getattr(query_metadata, field, [])
                if isinstance(values, list):
                    cleaned = [v for v in values if str(v).strip() not in _unbounded_values]
                    setattr(query_metadata, field, cleaned)
